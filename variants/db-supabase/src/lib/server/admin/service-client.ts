 ...
 *
 * It lives under `$lib/server/admin/` and ESLint refuses to let anything outside
 * that directory import it. That restriction is the point: on this branch RLS is
 * the authorization layer, so a query that skips it has skipped authorization,
 * and the set of places allowed to do that should be small enough to read in one
 * sitting.
 *
