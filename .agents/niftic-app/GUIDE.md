...
The workflow guide for any coding agent — Claude Code, Codex, Cursor, Aider or a
human reading over one's shoulder. `AGENTS.md` covers what this app _is_; this
covers how to change it.

`niftic.app.yml` decides which half applies. A `generated:` block means the repo
is a configured app; no block means it is still a template.
...
- On the Supabase fork: every table has RLS enabled, explicit policies, and a
  grant; the service-role client appears only under `src/lib/server/admin/`.
